...
static int rtcm_open(const char *tty)
{
	...
	memset(&t, 0, sizeof t);
	t.c_cc[VMIN] = 5;
	t.c_cflag = B9600 | CS8 | CREAD | CLOCAL | HUPCL; //baud rate, 8 bits, CREAD and CLOCAL are mandatory, let modem hang up
...
static void ubx_cfg_prt(int fd, bool UBX, bool NMEA, bool RTCM)
{
	...
	std::cout << (int)ubx_cfg_prt.outProtoMask << "\n";
...
static void ubx_cfg_uart(int fd, int id, unsigned int br, bool UBX, bool NMEA, bool RTCM)
{
	/* CFG-PRT */
	struct ubx_payload_tx_cfg_prt{
		unsigned char	portID;
	...
	ubx_cfg_prt.txReady                 = 0x00;      //0x25D1;0b0010010111010001;
	ubx_cfg_prt.mode[0]                 = 0xC0;      //0x25D1;0b0010010111010001;
	ubx_cfg_prt.mode[1]                 = 0x08;      //0x25D1;0b0010010111010001;
	ubx_cfg_prt.mode[2]                 = 0x00;      //0x25D1;0b0010010111010001;
	ubx_cfg_prt.mode[3]                 = 0x00;      //0x25D1;0b0010010111010001;	
	ubx_cfg_prt.baudrate                = br;        //0b11000000;
	ubx_cfg_prt.inProtoMask             = 0x01;      /*UBX*/
	ubx_cfg_prt.outProtoMask            = 0x00;
	if (UBX) ubx_cfg_prt.outProtoMask  |= 0x01;      /*UBX*/
	if (NMEA) ubx_cfg_prt.outProtoMask |= 0x02;      /*NMEA*/
...
static unsigned char getbyte(int fd, unsigned char rbuf[], unsigned char *rp, int bufcnt)
{
...
// 
	...
	// if(n < 75){
		std::cout << "Read is " << n << " and data is \n";
		if (space){  //UBX Message Parser
			while(a < n)
				getbyte(fd, buf, rp, n);
			// for(int i=0; i<n; i++){
...
template <typename T, bool space>
bool read_after_write(struct pollfd *pfd, double sleep_time){
	sleep(sleep_time);
	ros::spinOnce();
	if (!poll(pfd, 1, 3000)) {
...
int main(int argc, char **argv)
{
	...
	double buf[size], c;
	pnh.param<double>("c", c,654433023);
	...
	timenow = ros::Time::now().toSec();
  	...
  	while (ros::ok()) {
		...
		read_after_write<int,true>(&pfd[0],1);
		ubx_cfg_prt(pfd[0].fd, 1, 1, 0);
		read_after_write<int,true>(&pfd[0],0);
		ubx_cfg_uart(pfd[0].fd,1, 9600, 1,0,0); //disable NMEA
		read_after_write<int,true>(&pfd[0],1);

//		std::cin >> product_id;
			
		ubx_cfg_msg(pfd[0].fd, 0xF0, 0x00, 0); //Disable GGA
		read_after_write<int,true>(&pfd[0],1);
		ubx_cfg_msg(pfd[0].fd, 0xF0, 0x01, 0); //Disable GLL
		read_after_write<int,true>(&pfd[0],1);
		ubx_cfg_msg(pfd[0].fd, 0xF0, 0x02, 0); //Disable GSA
		read_after_write<int,true>(&pfd[0],1);
		ubx_cfg_msg(pfd[0].fd, 0xF0, 0x03, 0); //Disable GSV
		read_after_write<int,true>(&pfd[0],1);
		ubx_cfg_msg(pfd[0].fd, 0xF0, 0x04, 0); //Disable RMC
		read_after_write<int,true>(&pfd[0],1);
		ubx_cfg_msg(pfd[0].fd, 0xF0, 0x05, 0); //Disable VTG
		read_after_write<int,true>(&pfd[0],1);
		ubx_cfg_msg(pfd[0].fd, 0xF0, 0x0D, 1); //Enable GNS
		read_after_write<int,true>(&pfd[0],1);

		ubx_cfg_rate(pfd[0].fd, 200, 1);        //Set rate to 10Hz
	
		start_gps_stream(pfd[0].fd);
		read_after_write<int,true>(&pfd[0],0);

		ubx_cfg_uart(pfd[0].fd,1, 9600, 1,1,0);
		read_after_write<int,true>(&pfd[0],1);
		std::cin >> product_id;

		while(ros::ok()) {
			//sleep(1);
			if(!read_after_write<unsigned char, false>(&pfd[0],0.5))
				break;	
 	 	}
